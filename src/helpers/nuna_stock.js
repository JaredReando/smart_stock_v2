...
export default class NunaStock {
  ...
  getMaterialStockLevels = () => {
    ...
    const fixedBinDetails = this.fixedBinsJSON.reduce(
      (binDetails, fixedBin) => {
  ...
  sortInventoryByFifo(parsedInventoryRecords) {
    const fifoSortedInventory = [...parsedInventoryRecords].sort(
      (bin1, bin2) => {
        ...
        if (
          bin1['material'] === bin2['material'] &&
          bin1['storageBin'] > bin2['storageBin']
        )
          return 1;
        if (
          bin1['material'] === bin2['material'] &&
  ...
  getEmptyFixedBins() {
    ...
    let binsToRestockObjects = this.getFifoSortedInventory().filter(
      record =>
        (record.available == 0) & fixedBinNames.includes(record.storageBin)
    );
    //MAPS BACK AN ARRAY CONTAINING ONLY EMPTY FIXED BIN NAMES
    let binsToRestockStrings = binsToRestockObjects.map(
      record => record['storageBin']
    );
  ...
  getFixedBinRestockReport() {
    ...
    const validRestockInventory = this.getValidRestockInventory(
      fifoSortedInventory
    );
    ...
    emptyFixedBins.forEach(fixedBin => {
      ...
      if (sliceIndex !== -1) {
        ...
        const restockRecord = {
          ...
          available:
            fifoResult['available'] >= 0 ? fifoResult['available'] : 'N/A',
          isCompleted: false,
          isMissing: false,
          noneStocked:
            materialStockLevels[fifoResult['material']].binsStocked === 0,
          uuid: uuid(),
        };
